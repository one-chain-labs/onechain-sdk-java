...
final class TraitHelpers {
    ...
    static void serialize_vector_point(java.util.List<ZkPoint> value, com.novi.serde.Serializer serializer) throws com.novi.serde.SerializationError {
        serializer.serialize_len(value.size());
        for (ZkPoint item : value) {
            item.serialize(serializer);
        }
    }

    static java.util.List<ZkPoint> deserialize_vector_point(com.novi.serde.Deserializer deserializer) throws com.novi.serde.DeserializationError {
        long length = deserializer.deserialize_len();
        java.util.List<ZkPoint> obj = new java.util.ArrayList<ZkPoint>((int) length);
        for (long i = 0; i < length; i++) {
            obj.add(ZkPoint.deserialize(deserializer));
        }
        return obj;
    }
