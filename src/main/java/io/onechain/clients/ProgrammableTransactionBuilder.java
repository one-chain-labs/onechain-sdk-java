...
import io.onechain.bcsgen.Argument.GasCoin;
import io.onechain.bcsgen.Argument.Input;
import io.onechain.bcsgen.Argument.NestedResult;
import io.onechain.bcsgen.Argument.Result;
import io.onechain.bcsgen.CallArg.Object;
import io.onechain.bcsgen.CallArg.Pure;
import io.onechain.bcsgen.Command.MakeMoveVec;
import io.onechain.bcsgen.Command.MergeCoins;
import io.onechain.bcsgen.Command.MoveCall;
import io.onechain.bcsgen.Command.Publish;
import io.onechain.bcsgen.Command.SplitCoins;
import io.onechain.bcsgen.Command.TransferObjects;
import io.onechain.bcsgen.ObjectArg.ImmOrOwnedObject;
import io.onechain.bcsgen.ObjectArg.SharedObject;
import io.onechain.bcsgen.ProgrammableTransaction.Builder;
import io.onechain.bcsgen.TransactionData.V1;
import io.onechain.clients.BuilderArg.ForcedNonUniquePureBuilderArg;
import io.onechain.clients.BuilderArg.ObjectBuilderArg;
import io.onechain.clients.BuilderArg.PureBuilderArg;
...
public class ProgrammableTransactionBuilder {
    ...
    public Argument pureBytes(byte[] bytes, boolean forceSeparate) {
        ...
        CallArg exist = this.inputs.put(arg, builder.build());

        short index = 0;
        if (exist != null) {
            for (BuilderArg key : this.inputs.keySet()) {
                if (key.equals(arg)) {
                    break;
                }
                index++;
            ...
            index = (short) (this.inputs.size() - 1);
        }
        Argument.Input.Builder argBuilder = new Input.Builder();
        argBuilder.value = index;
    ...
    public Argument obj(ObjectArg objectArg) {
        ...
        CallArg exist = this.inputs.put(search, objectBuilder.build());
        short index = 0;
        if (exist != null) {
            for (BuilderArg key : this.inputs.keySet()) {
    ...
    public Argument pure(java.lang.Object value) {
        ...
        try {
            ...
            } else if (value instanceof OneChainAddress) {
                bytes = ((OneChainAddress) value).bcsSerialize();
    ...
    public Argument moveCall(
        ...
        List<Argument> bcsArguments = new ArrayList<>();
        for (int i = 0; i < arguments.size(); i++) {
            bcsArguments.add(to(arguments.get(i), i));
        }
...
//            .collect(Collectors.toList());
    ...
    private Argument to(CallArg arg, int index) {
        if (arg instanceof CallArgObjVec) {
            return this.makeObjVec(((CallArgObjVec) arg).getObjectArgs());
        }
     ...
     * @return the argument
     */
    public Argument transferArg(OneChainAddress recipient, Argument arg) {
     ...
     * @return the argument
     */
    public Argument transferArgs(OneChainAddress recipient, List<Argument> args) {
     ...
     * @return the argument
     */
    public Argument transferSui(OneChainAddress recipient, Long amount) {
    ...
    public Argument transferObject(
            OneChainAddress recipient, Tuple3<ObjectID, SequenceNumber, ObjectDigest> objectRef) {
    ...
    public Argument transferObjects(
            OneChainAddress recipient, List<Tuple3<ObjectID, SequenceNumber, ObjectDigest>> objectRefs) {
        Argument recipientArg = this.pure(recipient);
        List<Argument> arguments =
                objectRefs.stream()
                        .map(
     ...
     * @return the argument
     */
    public Argument payAllSui(OneChainAddress recipient) {
        Argument recArg = this.pure(recipient);
        Command.TransferObjects.Builder builder = new TransferObjects.Builder();
     ...
     * @param amounts    the amounts
     */
    public void paySui(List<OneChainAddress> recipients, List<Long> amounts) {
    ...
    public void pay(
            ...
            List<OneChainAddress> recipients,
            List<Long> amounts) {
        if (coins.isEmpty()) {
    ...
    private void payImpl(List<OneChainAddress> recipients, List<Long> amounts, Argument coin) {
        ...
        LinkedHashMap<OneChainAddress, List<Integer>> recipientMap = new LinkedHashMap<>();
        ...
        for (Entry<OneChainAddress, List<Integer>> entry : recipientMap.entrySet()) {
            OneChainAddress oneChainAddress = entry.getKey();
            List<Integer> splitSecondaries = entry.getValue();
            Argument recArg = pure(oneChainAddress);
    ...
    public static TransactionData newTransferSui(
            OneChainAddress recipient,
            OneChainAddress sender,
            Long amount,
