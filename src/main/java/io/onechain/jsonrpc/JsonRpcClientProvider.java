...
public abstract class JsonRpcClientProvider {
  ...
  public <T> CompletableFuture<T> callAndUnwrapResponse(
    ...
    this.call(request, url, typeOfT)
        .thenAccept(
            jsonRpc20Response -> {
              if (jsonRpc20Response.getError() != null) {
                final OneChainApiException e;
                if (jsonRpc20Response.getThrowable() != null) {
                  e = new OneChainApiException(jsonRpc20Response.getThrowable());
                  e.setError(jsonRpc20Response.getError());
                } else {
                  System.out.println(jsonRpc20Response.getError());
                  e = new OneChainApiException(jsonRpc20Response.getError());
                }
        ...
        .exceptionally(
            throwable -> {
              OneChainApiException e = new OneChainApiException(throwable);
              future.completeExceptionally(e);
              return null;
            });
    return future;
