...
import io.onechain.jsonrpc.JsonRpc20Response.Error;
...
public class OkHttpJsonRpcClientProvider extends JsonRpcClientProvider {
  ...
  private final ConcurrentHashMap<Long, Long> requestIdToSubscriptionIds =
                            ...
                            public void onMessage(@NotNull WebSocket webSocket, @NotNull String text) {
                                ...
                                if (null != reply.get("id")) {
                                    ...
                                    if (reply.get("error") != null) {
                                        ...
                                        replayFuture.completeExceptionally(new OneChainApiException(error));
                                    }
                                    ...
                                    Type type =
                                            reply.get("method").equals("suix_subscribeTransaction")
                                                    ? new TypeToken<TransactionBlockEffects>() {
                                            }.getType()
                                                    : new TypeToken<OneChainEvent>() {
                                            }.getType();
    ...
    public <T> Disposable subscribe(
            JsonRpc20Request request, Consumer<T> onNext, Consumer<OneChainApiException> onError) {
        ...
        Disposable disposable =
                subject
                        ...
                        .subscribe(
                                ...
                                throwable -> onError.accept(new OneChainApiException(throwable)));
...
//                            .url(String.format("%s%s", this.baseUrl, url))
                            .url(String.format("%s", this.baseUrl))
                            .post(requestBody)
                            .build();
     ...
     */
    @SuppressWarnings("unchecked")
    public CompletableFuture<JsonRpc20Response<?>> callAsync(
            JsonRpc20Request request, String url, Type typeOfT) {
        final CompletableFuture<JsonRpc20Response<?>> future = new CompletableFuture<>();
        final Request okhttpRequest;
        try {
